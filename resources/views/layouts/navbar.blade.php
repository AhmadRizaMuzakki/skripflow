@php
    ...
    $mahasiswaNeedsDosen = $user->needsDosenPembimbing();
@endphp
...
>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        ...
        @if ($user->isMahasiswa())
                @if ($mahasiswaNeedsDosen)
                    <span class="nav-link nav-link-disabled" title="Pilih dosen pembimbing terlebih dahulu">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    ...
                    <a href="{{ route('dosen-pembimbing.show') }}"
                       class="nav-link {{ request()->routeIs('dosen-pembimbing.*') ? 'nav-link-active' : '' }}">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        @elseif ($user->isAdmin())
            <a href="{{ route('admin.dosen.index') }}"
               class="nav-link {{ request()->routeIs('admin.dosen.*') ? 'nav-link-active' : '' }}">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342" />
                </svg>
                Kelola Dosen
            ...
            <a href="{{ route('admin.bimbingan.index') }}"
               class="nav-link {{ request()->routeIs('admin.bimbingan.*') ? 'nav-link-active' : '' }}">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
